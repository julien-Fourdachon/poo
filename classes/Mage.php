...
class Mage extends Character
{
    ...
    private function fireball(Character $target) {
        ...
        if ($rand <= $this->magicPoints) {
            ...
            $status = "{$this->name} lance une boule de feu à {$target->name}! Il reste {$target->getLifePoints()} points de vie à {$target->name} !";
            ...
            $status = "{$this->name} n'a plus de point de magie, et attaque {$target->name} au couteau (ahah)! Il reste {$target->getLifePoints()} points de vie à {$target->name} !";
        }
        return $status;
    }
